...
# ─────────────────────────────────────────────────────────────────────────────
#  MEDLAB  — patient requests a test; sees visible results
# ─────────────────────────────────────────────────────────────────────────────
def medlab(request):
    patient = get_session_patient(request)
    if not patient:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            entry.save()
            return redirect('/medlab/?submitted=1')
    else:
        form = MedLabUserForm()

    lab_records = MedLabModel.objects.filter(
        patient=patient,
        visible_to_patient=True,
    ).order_by('-report_date')

    return render(request, 'medlab_records.html', {
        'patient':     patient,
        'form':        form,
        'lab_records': lab_records,
    })
